...
- **Embeddings:** Ollama (default) or OpenAI. Stored in Qdrant with metadata (tenant, book, edition, section).
- **Metadata storage:** Postgres stores books, editions, sections, and full section text; Qdrant stores only vectors and payloads for search.
...
- **Filtering:** Filter by `book_id`, `edition_id`, and always by tenant. Qdrant payload filters enforce isolation.
...
- **Next.js web app:** Optional UI for upload, search, and query. Served on port 3001; configured with `NEXT_PUBLIC_API_URL` pointing at the API.
...
For a high-level view of the problem we solve and what you can feed in, see [Overview & scope](OVERVIEW_AND_SCOPE.md). For running the system, see [Getting started](GETTING_STARTED.md).
